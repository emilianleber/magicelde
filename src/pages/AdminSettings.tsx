...
const AdminSettings = () => {
  ...
  const [calSources, setCalSources] = useState<{ id: string; name: string; url: string; color: string; enabled: boolean; isNew?: boolean }[]>([]);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      const [templatesResult, signatureResult, settingsResult, tvResult, etResult, calResult] = await Promise.all([
      ...
      if (!calResult.error) setCalSources((calResult.data || []) as any[]);
      ...
      {activeTab === "kalender" && (() => {
        ...
        return (
          <div className="space-y-5">
            ...
            {/* ── Meine Kalender verbinden ── */}
            <div className="p-6 rounded-2xl bg-muted/20 border border-border/30">
              <div className="flex items-center justify-between mb-1">
                <h3 className="text-sm font-bold text-foreground">Meine Kalender</h3>
                <button
                  onClick={() => setCalSources(prev => [...prev, { id: "", name: "", url: "", color: "#6366f1", enabled: true, isNew: true }])}
              ...
              <p className="text-xs text-muted-foreground mb-4">
                Füge deine Kalender hinzu (Apple, Google, Outlook). Pro Kalender eine iCal-URL.
              </p>

              <div className="space-y-3">
                {calSources.length === 0 && (
                  <div className="p-4 rounded-xl border-2 border-dashed border-border/30 text-center text-sm text-muted-foreground">
                    Noch keine Kalender verbunden
                  </div>
                )}

                {calSources.map((src, idx) => (
                  <div key={src.id || `new-${idx}`} className="p-4 rounded-xl bg-background border border-border/20 space-y-2">
                    <div className="flex items-center gap-2">
                      <input
                        type="color"
                        value={src.color || "#6366f1"}
                        onChange={e => setCalSources(prev => prev.map((s, i) => i === idx ? { ...s, color: e.target.value } : s))}
                        className="w-6 h-6 rounded cursor-pointer border-0"
                      />
                      <input
                        value={src.name}
                        onChange={e => setCalSources(prev => prev.map((s, i) => i === idx ? { ...s, name: e.target.value } : s))}
                        placeholder="Kalendername (z.B. Privat, Arbeit)"
                        className="flex-1 rounded-lg bg-muted/30 border border-border/20 px-3 py-1.5 text-sm"
                      />
                      <label className="flex items-center gap-1 cursor-pointer shrink-0">
                        <input
                          type="checkbox"
                          checked={src.enabled}
                          onChange={e => setCalSources(prev => prev.map((s, i) => i === idx ? { ...s, enabled: e.target.checked } : s))}
                      ...
                      <button
                        onClick={async () => {
                          if (src.id) await supabase.from("calendar_sources").delete().eq("id", src.id);
                          setCalSources(prev => prev.filter((_, i) => i !== idx));
                        }}
                    ...
                    <input
                      value={src.url}
                      onChange={e => setCalSources(prev => prev.map((s, i) => i === idx ? { ...s, url: e.target.value } : s))}
                      placeholder="https://p123-caldav.icloud.com/published/..."
                      className="w-full rounded-lg bg-muted/30 border border-border/20 px-3 py-1.5 text-xs font-mono"
                    />
                  </div>
                ))}
              </div>
              ...
              <div className="flex items-center gap-2 mt-4">
                  ...
                  onClick={async () => {
                    ...
                    try {
                      for (const src of calSources) {
                        if (!src.name.trim() || !src.url.trim()) continue;
                        if ((src as any).isNew || !src.id) {
                          const { data } = await supabase.from("calendar_sources").insert({ name: src.name.trim(), url: src.url.trim(), color: src.color, enabled: src.enabled }).select("id").single();
                          if (data) src.id = data.id;
                        } else {
                          await supabase.from("calendar_sources").update({ name: src.name.trim(), url: src.url.trim(), color: src.color, enabled: src.enabled }).eq("id", src.id);
                        }
                      }
                      // Sync starten
                    ...
                    } catch (err: any) { setCalMsg("Fehler: " + err.message); }
                    setCalSaving(false);
                  }}
                  disabled={calSaving}
                ...
                >
                  {calSaving ? "Synchronisiert..." : "Speichern & Synchronisieren"}
                </button>
              </div>
              ...
              <div className="mt-4 p-3 rounded-xl bg-background/60 border border-border/10">
                ...
                <p className="text-[10px] text-muted-foreground"><strong>Apple:</strong> Kalender App → Rechtsklick auf Kalender → Teilen → "Öffentlicher Kalender" → URL kopieren (webcal:// durch https:// ersetzen)</p>
                <p className="text-[10px] text-muted-foreground mt-1"><strong>Google:</strong> Kalender-Einstellungen → "Geheime Adresse im iCal-Format"</p>
                <p className="text-[10px] text-muted-foreground mt-1"><strong>Outlook:</strong> Kalender → Einstellungen → Freigegebene Kalender → iCal-Link</p>
              </div>
            </div>
          </div>
        );
