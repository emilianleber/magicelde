        ...
        <div className="container px-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
            <div>
              ...
              <h1 className="font-display text-2xl md:text-3xl font-bold text-foreground">Willkommen, {displayName}</h1>
              {kundennummer && <p className="font-sans text-sm text-muted-foreground mt-1">Kundennummer: {kundennummer}</p>}
            </div>
            ...
            <div className="space-y-8">
              <div className="grid sm:grid-cols-3 gap-4">
                {[
                  ...
                  { label: "Nächstes Event", value: events[0]?.event_date ? new Date(events[0].event_date).toLocaleDateString("de-DE") : "–", icon: Clock },
                ].map((stat) => (
              ...
              {timeline.length > 0 && (
                <div className="p-8 rounded-3xl bg-muted/20 border border-border/30">
                  <h2 className="font-display text-lg font-bold text-foreground mb-6">Event-Timeline{events[0] ? `: ${events[0].title}` : ""}</h2>
                  <div className="space-y-4">
                    {timeline.map((t) => (
                      <div key={t.id} className="flex items-start gap-4">
                        <div className="mt-0.5">
                          {t.done ? <CheckCircle2 className="w-5 h-5 text-accent" /> : <Circle className="w-5 h-5 text-muted-foreground/30" />}
                        </div>
                        <div className="flex-1">
                          <p className={`font-sans text-sm font-medium ${t.done ? "text-foreground" : "text-muted-foreground"}`}>{t.step}</p>
                          {t.step_date && <p className="font-sans text-xs text-muted-foreground">{new Date(t.step_date).toLocaleDateString("de-DE")}</p>}
                        </div>
              ...
              {events.length === 0 && timeline.length === 0 && (
                <div className="p-12 rounded-3xl bg-muted/20 border border-border/30 text-center">
                  ...
                  <h3 className="font-display text-lg font-bold text-foreground mb-2">Noch keine Events</h3>
                  <p className="font-sans text-sm text-muted-foreground mb-6">Nach Ihrer Buchung werden hier Ihre Events und der aktuelle Status angezeigt.</p>
                    ...
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className={`font-sans text-[10px] uppercase tracking-widest px-2 py-1 rounded-full ${
                          ...
                          {event.status === "confirmed" ? "Bestätigt" : event.status === "completed" ? "Abgeschlossen" : "In Planung"}
                        </span>
                      </div>
                      <div className="flex flex-wrap gap-x-6 gap-y-1 font-sans text-sm text-muted-foreground">
                        {event.event_date && <span>📅 {new Date(event.event_date).toLocaleDateString("de-DE")}</span>}
                ...
                <div key={doc.id} className="flex items-center justify-between p-5 rounded-2xl bg-muted/20 border border-border/30 hover:border-accent/20 transition-colors">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-sans text-xs text-muted-foreground">{doc.type} · {new Date(doc.created_at).toLocaleDateString("de-DE")}</p>
                    </div>
                  </div>
                  {doc.file_url && (
