  ...
  return (
    <AdminLayout title={isNew ? "Neues Konzept" : name || "Konzept"} subtitle={FORMAT_OPTIONS.find(f => f.value === format)?.label || "Show-Ablauf planen"}>
      ...
      <div className="flex items-center justify-between mb-4">
        <button onClick={async () => { if (!isNew && name.trim()) await handleSave(); navigate(isNew ? "/admin/programm/shows" : `/admin/programm/shows/${id}`); }}
          className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
          <ArrowLeft className="w-4 h-4" /> {isNew ? "Zurück" : "Speichern & Schließen"}
        </button>
